...
export function RegistrationForm() {
  ...
  useEffect(() => {
    ...
    if (whoiam?.isAuthenticated) navigate('/');
  }, [navigate]);
  ...
  const onSubmit = async data => {
    ...
    try {
      const response = await axios.post(
        'https://todo-api-dcld.onrender.com/api/user/register',
        data,
        {
          headers: {
            ...
            {...register('password', {
              ...
              pattern: {
                value:
                  /(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[@#$%^&+!=]).{8,}/,
                message:
                  'Password must meet the criteria (e.g., at least one lowercase, one uppercase, one digit, one special character, and a minimum length of 8 characters)',
              },
            })}
            type="password"
            className="form-control"
        ...
        <div>
          <button
            ...
            {isloading ? (
              ...
              <>
                <i className="far fa-edit text-primary me-2"></i>
